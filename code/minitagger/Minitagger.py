...
class Minitagger(object):
	...
	def __report_performance(self, data_test, pred_labels):
		...
		@param data_test: contains the testing dataset
		...
		for words, labels in data_test.sequence_pairs:
			...
			for i in range(len(words)):
				# treat partially labeled data
				golden_label = labels[i] if labels[i] is not None else "<?>"
				# create line to print in the file
				line = words[i] + " " + golden_label + " " + pred_labels[sequence_number][i] + "\n"
				# write to file
				f1.write(line)
			# separate sentences with empty lines
			f1.write("\n")
			if labels != pred_labels[sequence_number] and not data_test.is_partially_labeled:
				for i in range(len(words)):
					# create line to print in the file
		...
		# report performance if data is not partially labelled
	...
	def __fit_crf(self, data_train):
		"""
		Fits CRF model on the given train data
		...
		self.__ml_model.fit(features_list, label_list)
	...
	def __predict_crf(self, data_test):
		"""
		Predicts using CRF
		...
		assert not self.__feature_extractor.is_training, "Training should be False for making predictions"
		assert data_test is not None, "Test data is necessary for prediction"
	...
	def __fit_svm(self, data_train):
		...
		self.__ml_model = liblinearutil.train(problem, liblinearutil.parameter("-q"))
	...
	def __predict_svm(self, data_test):
		"""
		...
		pred_labels, (acc, _, _), _ = liblinearutil.predict(label_list, features_list, self.__ml_model, "-q")
	...
	def __build_nn(self, num_features, hidden_units):
		...
		# define model
		self.__ml_model = Sequential()
		# initializer
		initializer = glorot_uniform(1234)
		# define input_dim and number of units in the next hidden layer (e.g. 32)
		self.__ml_model.add(Dense(hidden_units[0], input_dim=num_features, init=initializer, activation="relu", W_constraint=maxnorm(3)))
		if len(hidden_units) == 2:
			# add dropout
			self.__ml_model.add(Dropout(0.2, seed=1223))
			# add 2nd hidden layer
			self.__ml_model.add(Dense(hidden_units[1], init=initializer, activation="relu", W_constraint=maxnorm(3)))
		# define output layer with 3 classes and softmax activation function
		self.__ml_model.add(Dense(3, init=initializer, activation="softmax"))
		# Compile model
		adam = Adam(lr=0.001, decay=1e-3)
		self.__ml_model.compile(loss="binary_crossentropy", optimizer=adam, metrics=["accuracy"])

	def __fit_nn(self, data_train):
		"""
		Fits NN model on the given train data
		...
		# # Fit the model
		# self.__ml_model.fit(X_train, y_train, epochs=10, batch_size=16, verbose=1, validation_split=0.05, callbacks=[early_stop])

		# retrain model using the optimum number of epochs
		self.__build_nn(num_features, [128, 64])
		# self.__ml_model.fit(X_train, y_train, nb_epoch=early_stop.stopped_epoch+1, batch_size=64, verbose=1)
		self.__ml_model.fit(X_train, y_train, epochs=10, batch_size=16, verbose=1, shuffle=False)
		self.__feature_extractor.is_training = False

	def __predict_nn(self, data_test):
		"""
		Make predictions using NN and the test data
		...
		assert not self.__feature_extractor.is_training, "Training should be False for making predictions"
		assert data_test is not None, "Test data is necessary for prediction"
		# get number of data points from the training dataset
		num_features = self.__feature_extractor.num_feature_types()
		...
		# make predictions
		y_pred = self.__ml_model.predict(X_test)
	...
	def train(self, cv, data_train, data_test):
		...
		if cv:
			...
			# cast the dataset into np.array (necessary for using smart indexing and KFold)
			data_set = np.array(data_set)
			# initialize CRF object if CRF is used
			if self.classifier == "crf":
				self.__ml_model = sklearn_crfsuite.CRF(algorithm="lbfgs", all_possible_states=True, all_possible_transitions=True, c1=0.1, c2=0.1)
			# perform cross validation
			kf = KFold(n_splits=folds, random_state=6)
			# iterate through the dataset and create train and test sets
			for fold, (train, test) in enumerate(kf.split(data_set)):
				...
				if self.classifier == "svm":
					self.__fit_svm(data_train)
					self.__predict_svm(data_test)
				if self.classifier == "crf":
					self.__fit_crf(data_train)
					self.__predict_crf(data_test)
				if self.classifier == "nn":
					self.__fit_nn(data_train)
					self.__predict_nn(data_test)
	...
	def save(self, model_path):
		...
		if (self.__feature_extractor.feature_template == "relational") and (self.__feature_extractor.parser_type == "spacy"):
			...
			if self.__feature_extractor.classifier == "svm":
				liblinearutil.save_model(os.path.join(model_path, "ml_model"), self.__ml_model)
			else:
				pickle.dump(self.__ml_model, open(os.path.join(model_path, "ml_model"), "wb"))

	def load(self, model_path, classifier):
		"""
		...
		try:
			...
			# load trained model
			if classifier == "svm":
				self.__ml_model = liblinearutil.load_model(os.path.join(model_path, "ml_model"))
			else:
				self.__ml_model = pickle.load(open(os.path.join(model_path, "ml_model"), "rb"))
		except:
			raise Exception("No files found in the model path")

	def predict(self, data_test, classifier):
	...
	def __find_most_frequent_words(self, data_train):
		...
		@type data_train: SequenceData
		@param data_train: t# Train for the first time.he training dataset
	...
	def __train_silently(self, data_selected):
		...
		@param data_selected: the selected subset of the training dataset. Some words have correct labels
	...
	def __interval_report(self, data_selected, data_test, logfile):
		...
		if data_test is not None:
			...
			_, acc = self.__predict_svm(data_test)
			self.quiet = quiet_value
	...
	def train_actively(self, data_train, data_test):
		...
		@param data_train: list of training dataset
		@type data_test: list
		@param data_test: list of test dataset
		"""
		...
		while len(locations) < data_train.num_labeled_words:
			...
			_, _, scores_list = liblinearutil.predict(label_list, features_list, self.__ml_model, "-q")
